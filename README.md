...
> estudio de esta tesina. Ver detalle en
> [`informe/Informe_Tesina_Rubrica_Builder.md`](informe/Informe_Tesina_Rubrica_Builder.md),
> sección 12.
...
```
.
├── informe/            Informe de la tesina (md / html / pdf)
...
├── U1/ … U6/            Rúbricas por unidad de OE, con sus entregas sintéticas de testeo (sinteticos/)
├── TPI/                 Rúbrica y testeo del Trabajo Práctico Integrador
└── borradores/          Archivos de prueba manual previos al flujo ordenado por unidad (histórico, no autoritativo)
```

Cada carpeta `U*/sinteticos/<timestamp>/` contiene el rastro completo de una
...
|---|---|---|
| `U1/` | **TP1 — Primeros Pasos con Git** | Versionado, estados de archivos, `.gitignore`, ramas; flujo `init` → commits → rama de desarrollo → merge |
| `U2/` | **TP2 — Ingeniería de Prompts y Pensamiento Computacional** | Arquitectura de la instrucción (Instrucción/Contexto/Datos de Entrada/Indicador de Salida), parámetros de LLM (Temperatura, Top-p) y Predicción de Tokens, razonamiento Chain-of-Thought |
| `U3/` | **TP3 — Fundamentos de Metodologías Ágiles** | Waterfall vs. mentalidad ágil, roles de Scrum, Kanban y WIP Limit, Pair Programming (XP), Historias de Usuario con Criterios de Aceptación |
| `U4/` | **TP4 — El Ecosistema Startup y el Modelo Lean** | Startup vs. PyME (escalabilidad e incertidumbre), Lean Startup y MVP, Test de la Madre (Customer Discovery), métricas LTV vs. CAC y Churn Rate, modelos de negocio en software |
| `U5/` | **TP5 — Live Coding: El Arte de Programar en Tiempo Real** | Transparencia del proceso y explicación en paralelo en entrevistas técnicas, Boilerplate y Snippet Management, Hot Reloading, planificación con Checkpoints |
| `U6/` | **TP6 — Automatización de Flujos con n8n** | Ecosistema Low-Code, triggers vs. acciones, gestión de secretos, nodo Edit Fields, construcción de un workflow (`Schedule Trigger → Edit Fields → nodo de mensajería`) |
| `TPI/` | **TPI — Del MVP a la Automatización (Parte 2: Automatización y Control de Versiones)** | Construcción de un flujo n8n (`Webhook → Edit Fields` con expresión dinámica) + evidencia de uso de Git (`git log --oneline --stat`). La Parte 1 (teoría en PDF) se corrige manualmente y no forma parte de esta rúbrica |

> **Nota de numeración corregida:** las rúbricas de `U5` y `U6` tenían
> originalmente los títulos "TP4" y "TP5" (coincidiendo por error con el TP4 de
> `U4`). Se corrigieron a "TP5" y "TP6" para que el título interno del JSON
...
> Procesos), mientras que el campo `metadata.unidad` de algunas rúbricas (`U2` →
> "3", `U5` → "Unidad 6", `U6` → "Unidad 7") usa una numeración distinta, propia
...
- Material teórico y rúbricas de Organización Empresarial: producción propia de los autores de esta tesina.
